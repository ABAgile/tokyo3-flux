...
Use `flux_today` as the source for the complete group-scoped delivery status. Use `flux_changes` for the derived history of observations in an explicit time window, `flux_item_history` for one work item, `flux_snapshot` for a point-in-time reconstruction, and `flux_flow` for observation-based throughput counts. Use `flux_context` for confirmed human-reported delay explanations and scope changes; it is an explicitly labeled overlay, not GitLab evidence. Use `flux_report` for bounded standup, sprint-health, refinement, planning, backlog, and retrospective reports; `flux_standup`, `flux_sprint_health`, `flux_refinement`, `flux_planning`, `flux_backlog`, and `flux_retrospective` are focused aliases. Reports preserve GitLab evidence, observed history, human overlay provenance, coverage, and uncertainties. Use `flux_sprint_status` for a compact sprint-health summary with risks. Use `flux_triage`, `flux_review_queue`, and `flux_pipeline_failures` for focused read-only views. The report CLI path reads the locally reconciled snapshot; the authenticated report API path can read a selected milestone on demand. Other CLI fallback tools retain their documented direct/read-model behavior.
...
- Call `flux_report` with `kind=standup` or `kind=sprint_health` for evidence-based ceremony reporting; call it with `refinement`, `planning`, `backlog`, or `retrospective` for those views. `flux_standup` is a convenience alias for the standup report. Keep observed history and the `human_context_overlay` explicitly separate; do not invent changes outside the recorded window.
...
- Treat all Flux tools as read-only; they do not approve, assign, edit, retry, or otherwise mutate GitLab or human context.
- Treat `flux_context` entries as human-reported statements, not independently verified causes. Preserve coverage and uncertainties; never infer blame, performance, medical details, or protected characteristics.
