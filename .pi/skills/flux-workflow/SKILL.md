...
Use `flux_today` as the source for the complete group-scoped delivery status. Use `flux_changes` for the derived history of observations in an explicit time window, `flux_item_history` for one work item, `flux_snapshot` for a point-in-time reconstruction, and `flux_flow` for observation-based throughput counts. Use `flux_context` for confirmed human-reported delay explanations and scope changes; it is an explicitly labeled overlay, not GitLab evidence. Use `flux_sprint_status` for a compact sprint-health summary with risks. Use `flux_triage`, `flux_standup`, `flux_review_queue`, and `flux_pipeline_failures` for focused read-only views. The CLI path reconciles the active GitLab milestone; the API path reads Flux's authenticated read model.
