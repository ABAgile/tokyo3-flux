...
async (page) => {
 ...
 const projectSections = page.locator('.maintenance-sections'); check(await projectSections.evaluate(sections => sections.firstElementChild?.querySelector('h2')?.textContent === 'GitLab integration'),'GitLab integration should be shown first'); check(await projectSections.evaluate(sections => getComputedStyle(sections).gridTemplateColumns.split(' ').length === 1),'Projects page still uses a two-column layout'); const workspaceProjects = projectSections.locator(':scope > section').nth(1); check(await workspaceProjects.evaluate(section => !section.classList.contains('maintenance-section') && getComputedStyle(section).borderStyle === 'none'),'workspace projects still uses the maintenance container'); const projectFilterBar = page.locator('.project-filter-bar'); check(await projectFilterBar.count()===1 && await projectFilterBar.evaluate(bar => bar.previousElementSibling?.querySelector('h2')?.textContent === 'Workspace projects' && getComputedStyle(bar).display === 'flex'),'project filters are not in a horizontal bar below the title'); const projectFilterLabels = await projectFilterBar.locator('label').allTextContents(); check(projectFilterLabels.join('|')==='Assignee|Label|Search','project filters are in the wrong order'); const projectCount = projectFilterBar.locator('.project-filter-count'); check(await projectCount.count()===1,'project count is missing from the filter line');
 ...
 const projectFilter = page.getByRole('combobox',{name:'Project',exact:true}); const projectID = await projectFilter.inputValue(); const scopeFilter = page.getByRole('combobox',{name:'Scope',exact:true});
 ...
 const projectSummary = page.locator('#project-summary'); check(await projectSummary.isVisible(),'project summary is missing'); const projectSummaryText = await projectSummary.textContent(); for (const label of ['Remaining','Done','Blocked','Unscheduled','Active sprint coverage']) check(projectSummaryText.includes(label),`project summary is missing ${label}`);
 ...
 await page.reload(); await page.getByRole('heading',{name:'Planning list',exact:true}).waitFor(); check(await projectFilter.inputValue()===projectID && await scopeFilter.inputValue()==='all','reloading the project URL lost its filters');
 ...
 return 'PASS: workspace board, optional projects and project name/assignee/label filtering, sprint search, shared WIP, immutable item comments, card attachments, GitLab link controls, multi-sprint persistence, concurrent active sprints, closure isolation/history, stale edits, archive/restore, six accessible responsive layouts.';
}
