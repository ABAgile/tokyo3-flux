...
async (page) => {
 ...
 await listToggle.click(); await page.getByRole('heading',{name:'Planning list',exact:true}).waitFor(); check(await page.locator('.list-section').count()===initialColumnCount,'list does not preserve board column order/count'); const listHeaders = await page.locator('.list-table-head .list-table-heading').allTextContents(); check(listHeaders.join('|')==='Title|Project|People|Labels|Sprints|Links / Status','list table columns are missing'); check(await page.locator('.list-row .list-cell-people .participant-stack').count()===await page.locator('.list-row').count(),'list rows do not show the participant stack'); check(await page.locator('.list-row .list-cell-people .list-row-assignee-name').first().evaluate(node => node.textContent.trim().length > 0),'list rows lost the assignee name'); const listItemIDs = await page.locator('.list-row').evaluateAll(rows => { const ids = rows.map(row => row.dataset.item); return ids.length === new Set(ids).size; }); check(listItemIDs,'list duplicates work items'); check((await page.locator('.list-section').first().locator('.list-section-summary').textContent()).includes('shown ·'),'list section summary is missing filtered/WIP counts'); const firstListRow = page.locator('.list-row').first(); await firstListRow.focus(); await page.keyboard.press('Enter'); await page.locator('.item-detail-pane').waitFor(); check(await page.locator('.item-detail-pane').getByLabel('Title',{exact:true}).count()===1,'list selection did not open the detail pane'); check(await firstListRow.getAttribute('aria-current')==='true' && await firstListRow.evaluate(row => row.classList.contains('is-selected')),'selected list row is not marked'); const visibleListFields = await firstListRow.locator(':scope > .list-cell').evaluateAll(cells => cells.filter(cell => getComputedStyle(cell).display !== 'none').map(cell => cell.dataset.label)); check(visibleListFields.join('|')==='Title|Project|People|Labels','detail pane did not compact the list columns'); await page.keyboard.press('Escape'); check(await firstListRow.evaluate(row => row === document.activeElement),'detail close did not return focus to its originating row'); await boardToggle.click(); await page.getByRole('heading',{name:'Kanban board',exact:true}).waitFor();
 ...
 const projectSections = page.locator('#page-root > .page-stack'); check(await projectSections.evaluate(sections => sections.firstElementChild?.querySelector('h2')?.textContent === 'GitLab integration'),'GitLab integration should be shown first'); check(await projectSections.evaluate(sections => getComputedStyle(sections).gridTemplateColumns.split(' ').length === 1),'Projects page still uses a two-column layout'); const workspaceProjects = projectSections.locator(':scope > section').nth(1); check(await workspaceProjects.evaluate(section => !section.classList.contains('maintenance-section') && getComputedStyle(section).borderStyle === 'none'),'workspace projects still uses the maintenance container'); const projectFilterBar = page.locator('#content .workspace-projects .filter-bar'); check(await projectFilterBar.count()===1 && await projectFilterBar.evaluate(bar => bar.closest('.filter-slot')?.previousElementSibling?.querySelector('h2')?.textContent === 'Workspace projects' && getComputedStyle(bar).display === 'flex'),'project filters are not in a horizontal bar below the title'); const projectFilterLabels = await projectFilterBar.locator('label').allTextContents(); check(projectFilterLabels.join('|')==='Assignee|Label|Search','project filters are in the wrong order'); const projectCount = projectFilterBar.locator('.filter-bar-count'); check(await projectCount.count()===1,'project count is missing from the filter line');
 ...
 // Cards carry a participant stack: the assignee first behind a static accent
 // ring, then reviewers and commenters, all named for assistive technology.
 check(await page.locator('.card .card-meta > .participant-stack').count() === await page.locator('.card').count(),'cards do not carry a participant stack');
 const assigneeAvatar = page.locator('.card .participant-avatar.is-assignee').first();
 check(await assigneeAvatar.count()===1,'no card marks its assignee in the participant stack');
 check(await assigneeAvatar.evaluate(node => node === node.parentElement.firstElementChild && (node.getAttribute('aria-label') || '').includes('Assignee')),'the assignee is not first or not labelled in the stack');
 check(await assigneeAvatar.evaluate(node => { const style = getComputedStyle(node); return style.borderStyle !== 'none' && style.boxShadow !== 'none'; }),'the assignee has no static ring');
 check(await page.locator('.card .participant-stack').first().evaluate(stack => (stack.getAttribute('aria-label') || '').startsWith('Participants:') || (stack.getAttribute('aria-label') || '').startsWith('No participants')),'participant stack is unlabelled');
 ...
 await nav('Sprints'); const closeAgain = page.getByRole('article').filter({has:page.getByRole('heading',{name:'Sprint 1 · Planning foundations',exact:true})}); await closeAgain.getByRole('button',{name:'Close sprint',exact:true}).click(); await page.getByLabel('Closing decision / rationale').fill('Archive first sprint after review'); await page.getByRole('dialog').getByRole('button',{name:'Close sprint',exact:true}).click(); await saved();
 const archiveAgain = page.getByRole('article').filter({has:page.getByRole('heading',{name:'Sprint 1 · Planning foundations',exact:true})}); await archiveAgain.getByRole('button',{name:'Archive sprint',exact:true}).click(); await page.getByRole('dialog').getByRole('button',{name:'Archive sprint',exact:true}).click(); await saved(); await page.getByRole('heading',{name:'Archived sprint history',exact:true}).waitFor(); check(await page.locator('.sprint-history-list .maintenance-row').filter({hasText:'Sprint 1 · Planning foundations'}).count()===1,'archived sprint summary is missing'); check(await page.locator('.sprint-planning .sprint-panel').filter({has:page.getByRole('heading',{name:'Sprint 1 · Planning foundations',exact:true})}).count()===0,'archived sprint remained in working planning');
 await nav('Kanban board');
 await page.getByRole('combobox',{name:'Project',exact:true}).selectOption('none');
 ...
 return 'PASS: workspace board, multi-project associations and project name/assignee/label filtering, sprint search, shared WIP, immutable item comments, card attachments, GitLab link controls, multi-sprint persistence, concurrent active sprints, closure isolation/history, sprint archiving summaries, stale edits, archive/restore, six accessible responsive layouts, optimistic move rollback and single-board-read batches.';
}
