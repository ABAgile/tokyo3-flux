...
async page => {
 ...
 const trackedCard=page.locator(`[data-item="${item.id}"]`);await trackedCard.waitFor();await trackedCard.getByRole('button',{name:item.title,exact:true}).focus();await page.evaluate(()=>{window.scrollTo(0,Math.min(64,document.documentElement.scrollHeight-innerHeight));})
 const beforeRefresh=await page.evaluate(id=>{const card=[...document.querySelectorAll('[data-item]')].find(node=>node.dataset.item===id);window.__fluxRefreshCard=card;window.__fluxRefreshContent=document.querySelector('#content');window.__fluxRefreshScroll=scrollY;window.__fluxRefreshFocus=document.activeElement;return {scroll:scrollY,details:[...card.querySelectorAll('details')].map(node=>node.open)};},item.id);
 ...
 const afterRefresh=await page.evaluate(id=>{const card=[...document.querySelectorAll('[data-item]')].find(node=>node.dataset.item===id);return {sameCard:card===window.__fluxRefreshCard,sameContent:document.querySelector('#content')===window.__fluxRefreshContent,focus:document.activeElement===window.__fluxRefreshFocus,scroll:scrollY,details:[...card.querySelectorAll('details')].map(node=>node.open)};},item.id);check(afterRefresh.sameCard&&afterRefresh.sameContent,'background refresh rebuilt planning DOM');check(afterRefresh.focus&&afterRefresh.scroll===beforeRefresh.scroll&&JSON.stringify(afterRefresh.details)===JSON.stringify(beforeRefresh.details),'background refresh disturbed focus, scroll, or menus');
 await page.evaluate(()=>{window.__fluxContentMutations=0;window.__fluxContentObserver=new MutationObserver(records=>{window.__fluxContentMutations+=records.length;});window.__fluxContentObserver.observe(document.querySelector('#content'),{childList:true});});await page.waitForTimeout(16000);const unchangedMutations=await page.evaluate(()=>{window.__fluxContentObserver.disconnect();return window.__fluxContentMutations;});check(unchangedMutations===0,'unchanged observation poll replaced content children');
 ...
 const linkSection=page.locator(`[data-item="${item.id}"] .card-links-section`);check(await linkSection.getByText('GitLab links · 2',{exact:true}).count()===1&&await linkSection.getByRole('button',{name:/^View GitLab details/}).count()===1,'GitLab links are not grouped with one observations action');
 const observationIcon=page.locator(`[data-item="${item.id}"] [data-observation="status-icon"]`);await observationIcon.waitFor();const observationTitle=await observationIcon.getAttribute('title');const observationLabel=await observationIcon.getAttribute('aria-label');check(await observationIcon.getAttribute('tabindex')==='0'&&!observationTitle&&observationLabel?.startsWith('Card observation:'),'card observation icon is not hover/focus accessible');await observationIcon.hover();check(await observationIcon.evaluate(icon=>{const style=getComputedStyle(icon,'::after');const left=Number.parseFloat(style.left),top=Number.parseFloat(style.top),width=Number.parseFloat(style.width),height=Number.parseFloat(style.height);return style.visibility==='visible'&&style.position==='fixed'&&left>=0&&top>=0&&left+width<=innerWidth&&top+height<=innerHeight&&document.documentElement.scrollWidth<=innerWidth;}),'card observation tooltip is not visible within the viewport on hover');
