...
async page => {
 ...
 await openEditor(a);await page.getByLabel('Attachment file',{exact:true}).setInputFiles({name:'drag-preview.txt',mimeType:'text/plain',buffer:Buffer.from('drag preview')});await page.getByRole('status').filter({hasText:'Attachment uploaded.'}).waitFor();await close();
 const cardAttachments=card(a).locator('.card-attachments');await cardAttachments.locator('summary').click();await cardAttachments.locator('.card-attachment-option').waitFor();
 const mrIcon=card(a).locator('.card-observation-icon').first();await mrIcon.hover();check(await mrIcon.evaluate(icon=>getComputedStyle(icon,'::after').visibility==='visible'),'MR tooltip fixture did not become visible');
 await page.evaluate(()=>{window.__fluxNativeSetDragImage=DataTransfer.prototype.setDragImage;window.__fluxDragImages=[];DataTransfer.prototype.setDragImage=function(image,x,y){const icon=image.querySelector('.card-observation-icon');const original=Array.from(document.querySelectorAll('.card[data-item]')).find(node=>node!==image&&node.dataset.item===image.dataset.item);const imageAttachments=image.querySelector('.card-attachments');const originalAttachments=original?.querySelector('.card-attachments');const imageBounds=image.getBoundingClientRect();const originalBounds=original?.getBoundingClientRect();window.__fluxDragImages.push({isCard:image.matches('.card'),isPreview:image.dataset.dragPreview==='true',isolated:image.parentElement===document.body&&!image.querySelector('.card,.list-row'),hasMR:!!image.querySelector('.card-links-section'),hasAttachments:!!imageAttachments,attachmentsOpen:imageAttachments?.open===originalAttachments?.open,dragSource:image.classList.contains('drag-source'),sameSize:!!originalBounds&&Math.abs(imageBounds.width-originalBounds.width)<1&&Math.abs(imageBounds.height-originalBounds.height)<1,offsetValid:x>=0&&y>=0&&x<imageBounds.width&&y<imageBounds.height,tooltipDisplay:icon?getComputedStyle(icon,'::after').display:'none'});return window.__fluxNativeSetDragImage.call(this,image,x,y);};});
 let rejectedMoves=0;await page.route('**/changes',route=>{if(route.request().postDataJSON().kind==='item.move'){rejectedMoves++;return route.fulfill({status:409,contentType:'application/json',body:JSON.stringify({error:'planning changed; refresh and review before saving'})});}return route.continue();});
 const verifyDragImage=async expectedOpen=>{const rejection=page.waitForResponse(response=>response.url().endsWith('/changes')&&response.status()===409);await mrIcon.dragTo(card(b));await rejection;await page.getByRole('alert').filter({hasText:'planning changed'}).waitFor({timeout:15000});const dragImage=await page.evaluate(()=>window.__fluxDragImages.shift());check(dragImage?.isCard&&dragImage.isPreview&&dragImage.isolated&&dragImage.hasMR&&dragImage.hasAttachments&&dragImage.attachmentsOpen===expectedOpen&&dragImage.dragSource&&dragImage.sameSize&&dragImage.offsetValid&&dragImage.tooltipDisplay==='none','drag preview was not an isolated, correctly sized snapshot of the source card');};
 await verifyDragImage(true);check(rejectedMoves===1,'dragging the expanded card did not issue a move');await cardAttachments.locator('summary').click();await verifyDragImage(false);check(rejectedMoves===2,'dragging the collapsed card did not issue a move');
 await page.evaluate(()=>{DataTransfer.prototype.setDragImage=window.__fluxNativeSetDragImage;delete window.__fluxNativeSetDragImage;delete window.__fluxDragImages;});await page.unroute('**/changes');
